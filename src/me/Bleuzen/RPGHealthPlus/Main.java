	...
	public int getGroupsMaxHP(Player p) {
		...
		if(useGroupsPermissions) {
			if(!p.hasPermission("rpghealth.lvl.max")) {
				int set;
				int sum = getstartinghp + getmaxhp;
				if(p.hasPermission("rpghealth.lvl.high")) {
					set = (int) (sum / 1.5);
				}
				else if(p.hasPermission("rpghealth.lvl.med")) {
					set = (int) (sum / 2.0);
				}
				else if(p.hasPermission("rpghealth.lvl.low")) {
					set = (int) (sum / 2.5);
				}
				else if(p.hasPermission("rpghealth.lvl.lowest")) {
					set = (int) (sum / 3.0);
				}
				else {
					set = getstartinghp;
